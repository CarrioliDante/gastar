...
import { monthlyStats } from "@/data/mock";
import { fmt } from "@/lib/utils";

export function SavingsCard() {
  const pct = Math.round((monthlyStats.savings / monthlyStats.savingsGoal) * 100);
  const R = 36;
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginTop: 16 }}>
        ...
        <div>
          <p style={{ color: "#111111", fontSize: 28, fontWeight: 200, letterSpacing: "-1px", lineHeight: 1 }}>
            {fmt.currency(monthlyStats.savings)}
          </p>
          <p style={{ color: "rgba(0,0,0,0.35)", fontSize: 11, marginTop: 5, letterSpacing: "-0.2px" }}>
            of {fmt.currency(monthlyStats.savingsGoal)} goal
          </p>
