...
export async function middleware(request: NextRequest) {
  ...
  // Logged in + auth page → dashboard or onboarding
  if (user && isAuthRoute) {
    const url = request.nextUrl.clone();
  ...
  // Logged in + dashboard (not onboarding) → check onboarding
  if (user && !isOnboarding && !isAuthRoute && !isApi) {
    const needsOnboarding = !user.user_metadata?.onboarding_completed;
    if (needsOnboarding) {
      const url = request.nextUrl.clone();
  ...
  // Already done onboarding but goes to /onboarding
