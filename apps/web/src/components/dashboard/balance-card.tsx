...
import type { BalanceData, MonthlyStats } from "@gastar/shared";
...
export function BalanceCard({ balance, monthly, onQuickAdd }: Props) {
  return (
    <motion.div
    ...
    >
      ...
      >
        {[
          { label: "Income",   value: fmt.currency(monthly.income, true) },
          { label: "Spending", value: fmt.currency(monthly.spending, true) },
          { label: "Saved",    value: fmt.currency(monthly.savings, true) },
        ].map((stat, i) => (
          <div
            key={stat.label}
            style={{
          ...
          <div style={{ marginLeft: "auto", paddingLeft: 20 }}>
            <div style={{ display: "inline-flex", alignItems: "center", background: "rgba(255,255,255,0.07)", borderRadius: 20, padding: "5px 10px" }}>
              <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, letterSpacing: "-0.2px" }}>
                {balance.change > 0 ? "+" : ""}{balance.change}% this month
              </span>
            </div>
          </div>
        )}
