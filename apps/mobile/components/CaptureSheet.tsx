...
export function CaptureSheet({ open, initialType = 'expense', onClose, onSave }: CaptureSheetProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      translateY.value = withSpring(SCREEN_H, { damping: 20, stiffness: 300 }, () => {
  ...
  return (
    <Modal transparent animationType="none" visible={visible} onRequestClose={onClose}>
      ...
      <Pressable
        onPress={onClose}
        style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, backgroundColor: isDark ? 'rgba(0,0,0,0.55)' : 'rgba(0,0,0,0.2)' }}
      />
      ...
      >
        ...
        {saved ? (
          <View style={{ paddingVertical: 36, alignItems: 'center' }}>
            ...
            <Text style={{ fontFamily: fontDisplay, fontSize: 24, fontWeight: '500', letterSpacing: -1, color: C.ink, marginBottom: 8 }}>
              {isExp ? 'Anotado' : 'Recibido'}
            </Text>
            <Text style={{ fontFamily: fontMono, fontSize: 10, color: C.mute, letterSpacing: 1.2, textTransform: 'uppercase' }}>
